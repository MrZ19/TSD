...
import numpy as np
from models.zy import CapNet,Assign,Transformer
...
class KPFCNN(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.scores = CapNet(config)
        self.transformer_0 = Transformer(config,layer=0)
        self.transformer_1 = Transformer(config,layer=1)
        self.transformer_2 = Transformer(config,layer=2)
        self.transformer_3 = Transformer(config,layer=3)
        self.transformer_4 = Transformer(config,layer=4)
        self.assign=Assign()
        return
    ...
    def forward(self, batch):
        ...
        for block_i, block_op in enumerate(self.encoder_blocks):
            if block_i in self.encoder_skips:
                skip_x.append(x)
            x = block_op(x, batch)
            if block_i + 1 in self.encoder_skips:
                feature_encoder.append(x)

        for block_i, block_op in enumerate(self.decoder_blocks):
            if block_i == 0:
                feature_decoder.append(x)
            if block_i in self.decoder_concats:
                x = torch.cat([x, skip_x.pop()], dim=1)
            x = block_op(x, batch)
            if block_i in self.decoder_concats:
                feature_decoder.append(x)
        # cross information between src and tgt using transformer, in each layer of feature_decoder
        pair_information = []
        pair_information_0 = self.transformer_0(batch, feature_decoder)
        pair_information.append(pair_information_0)
        pair_information_1 = self.transformer_1(batch, feature_decoder)
        pair_information.append(pair_information_1)
        pair_information_2 = self.transformer_2(batch, feature_decoder)
        pair_information.append(pair_information_2)
        pair_information_3 = self.transformer_3(batch, feature_decoder)
        #pair_information_3 = feature_decoder[3]
        pair_information.append(pair_information_3)
        # pair_information_4 = self.transformer_4(batch, feature_decoder)
        # pair_information.append(pair_information_4)

        # summary pair information and layer information
        summary_features = [] 
        for i, feat in enumerate(feature_decoder):
            if i == len(feature_decoder)-1:
                summary_features.append(feat)
            else:
                summary_feature = feat + pair_information[i]
                summary_features.append(summary_feature)

        #feature_decoder = pair_information + feature_decoder
        #x = self.assign(self.config.assignment, batch, summary_features) # cross information between different layer by attention-metric or dynamic routing
        feature_decoder_s = []
        feature_decoder_t = []
        #feature_augment = []
        for i in range(len(batch['points'])):
            N_src, N_tgt = batch['stack_lengths'][4-i]
            # feature_decoder_s.append(feature_decoder[i][:N_src])
            # feature_decoder_t.append(feature_decoder[i][N_src:])
            feature_decoder_s.append(summary_features[i][:N_src])
            feature_decoder_t.append(summary_features[i][N_src:])
        x_s = self.assign(self.config.assignment, batch, feature_decoder_s, 0)
        x_t = self.assign(self.config.assignment, batch, feature_decoder_t, 1)

        feature_augment = torch.cat((x_s, x_t),dim=0)
        scores = self.detection_scores(batch, feature_augment)
        #features, scores = self.scores(batch, x)
        features = F.normalize(x, p=2, dim=-1)

        return features, scores

    def squash_feature_to_score(self, input_tensor):
        ...
        return torch.norm(output_tensor,dim=-1).permute(1,0)
