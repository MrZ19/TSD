...
from vis import vis_feature_distribution
...
def generate_feature_register_one_scene(inlier_ratio_threshold, distance_threshold, model, dloader, config, chose_snapshot):
    ...
    for i in tqdm(range(num_iter)):
        ...
        if args.random_points:
            ...
            source_indices = np.argsort(src_scores)[-args.num_points:]
            target_indices = np.argsort(tgt_scores)[-args.num_points:]
        ...
        gt_trans = inputs['corr'].detach().cpu().numpy()

        if True:
            vis_feature_distribution(source_keypts, source_desc, target_keypts, target_desc, gt_trans)
...
if __name__ == '__main__':
    ...
    result = generate_feature_register_one_scene(args.inlier_ratio_threshold, args.distance_threshold, model.cuda(), dloader, config, args.chosen_snapshot)
